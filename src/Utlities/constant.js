const Alert = ({ title, messege }) => {
  return (
    <div
      className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded absolute z-30 w-fit right-0"
      role="alert"
    >
      <strong className="font-bold mr-2">{title}</strong>
      <span className="block sm:inline mr-6">{messege}</span>
...
export const SuccessAlert = ({title,messege}) => {
  return (
    <>
      <div
